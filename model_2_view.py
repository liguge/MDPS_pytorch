...
def load_data(step, sample_len):
    ...
    train_loader = da.DataLoader(Train, batch_size=256, shuffle=True)
    validate_loader = da.DataLoader(Validate, batch_size=256, shuffle=True)
    test_loader = da.DataLoader(Test, batch_size=256, shuffle=False)
...
def train(train_loader, validate_loader, test_loader):
    res = []
...
def run_step():  # epoch=10
    ...
    for i in step_len:
        ...
        acc = train(sample1, sample1_validate, sample1_test)
        print(acc)
